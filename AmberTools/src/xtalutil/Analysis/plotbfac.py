...
#~ lines2=ax.plot(data1[:,0], data2[:,1],'b-')
#~ lines3=ax.plot(data1[:,0], data3[:,1],'#2FF101')
lines4=ax.plot(data4[:,0], data4[:,1],'m-')
lines5=ax.plot(data4[:,0], data5[:,1],'r-')
lines6=ax.plot(data4[:,0], data6[:,1],'c-')
...
#plt.xlabel('Atom',fontsize=28, labelpad=10)
plt.ylabel('B-factor',fontsize=28, labelpad=5)
st=fig.suptitle(r"%s Mean per-residue B-factors " %sys.argv[1], fontsize=28)
#plt.title('',fontsize=28)
...
from matplotlib.font_manager import fontManager, FontProperties
font=FontProperties(size=18)
ax.legend([r"crystal", r"revsym",r"rmsd"],bbox_to_anchor=(0, 0.0, .95, .95),prop=font)
