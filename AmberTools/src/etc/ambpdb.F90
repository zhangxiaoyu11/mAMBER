...
subroutine getnam0(natom,nres,mbona,nf)
   ...
   character(len=4) :: ititl(20)
   character(len=80) :: fmt,fmtin,ifmt,afmt,rfmt,type
...
subroutine getnam(natom,nres,igraph,ipres,lbres,ititl,nf, &
   ...
   character(len=4), intent(out) :: igraph(*), lbres(*), ititl(20), &
...
subroutine genpdb(natom,nres,coords,igraph,ipres,lbres,ititl,nf,arg1, &
   ...
   character(len=4), intent(in) :: igraph(*), ititl(20), &
                                   residue_chainid(nres), &
                                   atom_altloc(natom), &
                                   atom_element(natom), &
                                   residue_icode(nres)
   ...
   integer :: i, iat, imol, isrn, itype, &
              j, j1, j2, jc, jpat,j_print, &
              k, k_print, m, nn
   real :: elrad(15)   
   double precision :: xs, ys, zs
   character(len=1) :: type
   character(len=3) :: element, resnam
         ...
         if (arg1 .eq. '-pdb') then
            if (ext_pdb_data) then
               write(nf, &
                     ...
                     residue_icode(j)(1:1),coords(jc+1:jc+3),1.0,0.0, &
                     atom_element(k)
            else if (remediate) then
   ...
   return
    ...
    81 format('ATOM',2X,I5,1X,A4,1X,A4,1X, I4,4X,3F8.3,f8.4,f8.3,7X,A3)
...
subroutine genmol2(natom, nbond, nres, igraph, isymbl, coords, lbres, &
   ...
   character (len=4), intent(in) :: igraph(*), isymbl(*), lbres(*), ititl(*)
