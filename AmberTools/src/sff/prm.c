...
/* JMS -- Stop exiting if EOF is reached. This imposes the unnecessary condition
 * that a topology file ends in a newline.
 */
...
static void preadln(FILE * file, char *name, char *string)
{
   ...
   if (get_mytaskid() == 0) {
      for (i = 0; i < 81; i++) {
         if ((j = getc(file)) == EOF) {
            fprintf(nabout, "Error: EOF when searching for %s\n", name);
            ier = -1;
            break;
         } else {
            string[i] = (char) j;
...
static int pfind(FILE * file, int newparm, char *label)
{
   char line[81];
...
/* preadln2() - read to end-of-line; replace \n with \0. Non-fatal if we
 * hit EOF, though. Return 0 for EOF, 1 otherwise
*/

static int preadln2(FILE *file, char *name, char *string)
{
   int i, j, returnval = 0;
...
#if defined(MPI) || defined(SCALAPACK)
   MPI_Bcast(&returnval, 1, MPI_INT, 0, MPI_COMM_WORLD);
   // Don't broadcast the string if we don't have anything to report...
   if (returnval == 0) return returnval;
   // Otherwise broadcast our string
   MPI_Bcast(&i, 1, MPI_INT, 0, MPI_COMM_WORLD);
   MPI_Bcast(string, i, MPI_CHAR, 0, MPI_COMM_WORLD);
#endif
   // We are in error here, so return EOF-equivalent (we won't get here, anyway,
...
// This is a non-fatal version of pfind in that it will not bail out with an
// error if the requested flag cannot be found. This is necessary for topology
// file sections that are not necessary (e.g., SCEE_SCALE_FACTOR, ATOMIC_NUMBER,
// etc.)
//
static int pfind2(FILE * file, int newparm, char *label) {
   ...
   // Rewind the file so we can seek through it completely
   if (get_mytaskid() == 0)
      rewind(file);

   while (preadln2(file, label, line)) {
      if (!strncmp(line + 6, label, strlen(label))) {
...
PARMSTRUCT_T* rdparm( STRING_T* name )
{
   ...
   prm->Scee = (REAL_T *) get(sizeof(REAL_T) * prm->Nptra);
   prm->Scnb = (REAL_T *) get(sizeof(REAL_T) * prm->Nptra);
   ...
   prm->AtomicNum = (int *) get(sizeof(int) * prm->Natom);
   ...
   prm->N14sceelist = (REAL_T *) get(sizeof(REAL_T) * 10 * prm->Natom);
   prm->N14scnblist = (REAL_T *) get(sizeof(REAL_T) * 10 * prm->Natom);
   iptmp = (int *) get(sizeof(int) * 12 * prm->Natom);
   dihtmp = (int *) get(sizeof(int) * 12 * prm->Natom);
   ...
   if (newparm) {
      /*
       * READ SCEE SCALE FACTOR (Scee)
       */
      if (pfind2(file, newparm, "SCEE_SCALE_FACTOR")) {
         if (get_mytaskid() == 0) {
            for (i = 0; i < prm->Nptra; i++) {
#ifdef NAB_DOUBLE_PRECISION
...
#endif
               prm->Scee[i] = 1.0 / prm->Scee[i];
            }
         }
         skipeoln(file);
    } else {
         for (i = 0; i < prm->Nptra; i++)
            prm->Scee[i] = (REAL_T) (1.0 / 1.2);
      }
      ...
      /*
       * READ SCNB SCALE FACTOR (Scnb)
       */
   
      if (pfind2(file, newparm, "SCNB_SCALE_FACTOR")) {
         if (get_mytaskid() == 0) {
            for (i = 0; i < prm->Nptra; i++) {
#ifdef NAB_DOUBLE_PRECISION
...
#endif
               prm->Scnb[i] = 1.0 / prm->Scnb[i];
            }
         }
         skipeoln(file);
    } else {
         for (i = 0; i < prm->Nptra; i++)
            prm->Scnb[i] = (REAL_T) (1.0 / 2.0);
      }
       ...
       * READ ATOMIC_NUMBER
       */
      if (pfind2(file, newparm, "ATOMIC_NUMBER")) {
         if (get_mytaskid() == 0) {
            for (i = 0; i < prm->Natom; i++)
               fscanf(file, " %d", &prm->AtomicNum[i]);
         }
         skipeoln(file);
    } else {
         for (i = 0; i < prm->Natom; i++) {
            // Use the atomic masses to find the element
            prm->AtomicNum[i] = get_atomic_number(prm->Masses[i]);
         }
      }

#if defined(MPI) || defined(SCALAPACK)
      MPI_Bcast(prm->AtomicNum, prm->Natom, MPI_INTEGER, 0, MPI_COMM_WORLD);
#endif

 } else { // old parm
      // Fill up with the default scaling parameters
      for (i = 0; i < prm->Nptra; i++) {
         prm->Scee[i] = (REAL_T) (1.0 / 1.2);
         prm->Scnb[i] = (REAL_T) (1.0 / 2.0);
      }
...
#   endif
   ...
   for (i = 0; i < prm->Nphih; i++) {
      ...
      if (kat >= 0 && lat >= 0) {
         ...
         dihtmp[12 * ismall + prm->N14pairs[ismall]++] = prm->DihHNum[i] - 1;
      }
   }
   for (i = 0; i < prm->Mphia; i++) {
      ...
      if (kat >= 0 && lat >= 0) {
         ...
         ibig = iat > lat ? iat : lat;
         iptmp[12 * ismall + prm->N14pairs[ismall]] = ibig;
         dihtmp[12 * ismall + prm->N14pairs[ismall]++] = prm->DihNum[i] - 1;
      }
   }
   idum = 0;
   for (i = 0; i < prm->Natom - 1; i++) {
      for (k = 0; k < prm->N14pairs[i]; k++) {
         ...
         prm->N14sceelist[idum] = prm->Scee[dihtmp[12 * i + k]];
         prm->N14scnblist[idum++] = prm->Scnb[dihtmp[12 * i + k]];
      }
   }
#ifdef PRINT_14PAIRS
   if (get_mytaskid() == 0) {
...
INT_T wrparm( PARMSTRUCT_T* prm, STRING_T* name )
{
   ...
   if (get_mytaskid() == 0) {
      ...
      if (ier >= 0) {
         ...
         /* -16a- 1-4 Electrostatic scaling factor */
         FortranFormat(1, "%-80s");
         FortranWriteString("%FLAG SCEE_SCALE_FACTOR");
         FortranWriteString("%FORMAT(5E16.8)");
         FortranFormat(5, DBLFORMAT);
         for (i = 0; i < prm->Nptra; i++) {
            FortranWriteDouble(1.0 / prm->Scee[i]);
         }
         FortranEndLine();

         /* -16b- 1-4 van der Waals scaling factor */
         FortranFormat(1, "%-80s");
         FortranWriteString("%FLAG SCNB_SCALE_FACTOR");
         FortranWriteString("%FORMAT(5E16.8)");
         FortranFormat(5, DBLFORMAT);
         for (i = 0; i < prm->Nptra; i++) {
            FortranWriteDouble(1.0 / prm->Scnb[i]);
         }
         FortranEndLine();
...
PARMSTRUCT_T *copyparm(PARMSTRUCT_T * prm)
{
   ...
   newprm->Pk = prm->Pk;
   newprm->Pn = prm->Pn;
   newprm->Scee = prm->Scee;
   newprm->Scnb = prm->Scnb;
   ...
   newprm->N14sceelist = prm->N14sceelist;
   newprm->N14scnblist = prm->N14scnblist;
