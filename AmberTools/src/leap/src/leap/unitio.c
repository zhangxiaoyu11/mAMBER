            ...
            for (cmaplstt = cmaplst; cmaplstt->next != NULL; cmaplstt = cmaplstt->next)
              {  cmap=cmaplstt->cmap;
                 ...
                 for (j = 0; j < nprospect; j++) {
                     ...
                     if ( l == 4 ) ires = cmpresname1(uUnit, sb4[3], cmap->reslist, cmap->nres);
                     if (ires > 0) {
...
void
zUnitIOSaveAmberParmFormat(UNIT uUnit, FILE * fOut, FILE * fCrd,
 ...
 */
    ...
    for (i = 0; i < iVarArrayElementCount(uUnit->vaAtoms); i++) {
        ...
        if( GDefaults.iGBparm < 3  || GDefaults.iGBparm == 6 || GDefaults.iGBparm == 8 ) {
			...
			if( !(strcmp(cPTemp, "ASP") ||
                              strncmp(sAtomName(saPAtom->aAtom),"OD",2)) ||
                            !(strcmp(cPTemp, "GLU") ||
                              strncmp(sAtomName(saPAtom->aAtom),"OE",2)) ||
...
void
zUnitIOSaveAmberParmFormat_old( UNIT uUnit, FILE *fOut, FILE *fCrd, 
        BOOL bPolar, BOOL bPert )
{
int             i, iMax, iIndex;
LOOP            lTemp, lSpan;
