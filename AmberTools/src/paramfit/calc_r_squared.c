...
#pragma omp parallel for reduction(+:sum_of_x_squareds, sum_of_y_squareds, sum_x, sum_y, sum_xy) private(x_value, y_value, atom)
  for (point=0; point<global_options->NSTRUCTURES; ++point)
  {
    if (global_options->FUNC_TO_FIT==SUM_SQUARES_AMBER_STANDARD)
    {
      ...
      x_value=eval_amber_std_for_single_struct(global_options, parm_data, &coords_data[point]) + global_options->K;
    ...
    else if (global_options->FUNC_TO_FIT=AMBER_FORCES)
    {
      ...
      eval_amber_forces_single_struct(global_options, parm_data, coords_data, single_eval, point);
      x_value = y_value = 0.0;
      for (atom=0; atom<global_options->NATOMS; ++atom) {
        if (parm_data->fit_atom[atom] == YES) {
          ...
          y_value += abs(coords_data[point].force[atom].x) + abs(coords_data[point].force[atom].y) + abs(coords_data[point].force[atom].z);
        }
      }
      free(single_eval);
    }
