...
_unbuf_stdout = os.fdopen(sys.stdout.fileno(), 'w', 0)  # unbuffered stdout
_unbuf_stderr = os.fdopen(sys.stderr.fileno(), 'w', 0)  # unbuffered stderr
_stdout = sys.stdout
_stderr = sys.stderr
_debug_printlevel = 2
_mpi_size = 1
...
class MMPBSA_App(object):
   ...
   def _load_calc_list(self, prefix, mutant, parm_system):
      ...
      progs = {'gb' : self.external_progs['mmpbsa_py_energy'].full_path,
               ...
               'qh' : self.external_progs['ptraj'].full_path,
      ...
      if self.INPUT['entropy']:
         ...
         c = QuasiHarmCalc(progs['qh'], parm_system.complex_prmtop,
                           ...
                           '%sptrajentropy.in' % prefix,
                           '%sptraj_entropy.out' % prefix,
   ...
   def loadcheck_prmtops(self):
      ...
      if INPUT['alarun']:
         if (FILES.mutant_receptor_prmtop is None and 
             FILES.mutant_ligand_prmtop is None):
   ...
   def process_input(self):
      ...
      calculation (i.e., not re-writing output). This does the following prep:
         ...
         - Set temperature. Don't put it in namelist, because temp change
           for entropy requires changes to nmode and ptraj calcs, meaning it
   ...
   def parse_output_files(self):
      ...
      if INPUT['entropy']:
         if not INPUT['mutant_only']:
            self.calc_types['qh'] = QHout(self.pre + 'ptraj_entropy.out', 
                                          INPUT['temp'])
         if INPUT['alarun']:
            self.calc_types['mutant']['qh'] = QHout(self.pre + 
                                      'mutant_ptraj_entropy.out', INPUT['temp'])
