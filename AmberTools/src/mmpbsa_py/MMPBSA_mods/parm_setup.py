...
class MMPBSA_System(object):
   ...
   def __init__(self, complex_prmtop, receptor_prmtop=None, ligand_prmtop=None):
      ...
      self.complex_prmtop = AmberParm(complex_prmtop)
      self.stability = not receptor_prmtop and not ligand_prmtop
      if not self.stability:
         self.receptor_prmtop = AmberParm(receptor_prmtop)
         self.ligand_prmtop = AmberParm(ligand_prmtop)
      else:
      ...
      while i < self.complex_prmtop.ptr('nres'):
         ...
         if not self.res_list[i].selected:
            ...
            while i < self.complex_prmtop.ptr('nres') and \
               ...
               i += 1
            mask += ',' + range_string(com_start, i)
            com_start = -1
            i += 1

      # end while i < ...ptr(nres)
   ...
   def _validate(self):
      """ Validates the passed prmtops. Does a preliminary compatibility 
          check 
      """
      if not self.complex_prmtop.valid:
         raise PrmtopError('Complex prmtop %s invalid!' % self.complex_prmtop)
      
      if self.stability: return None # done checking here for stability calcs

      if not self.receptor_prmtop.valid:
         raise PrmtopError('Receptor prmtop %s invalid!' % self.receptor_prmtop)

      if not self.ligand_prmtop.valid:
         raise PrmtopError('Ligand prmtop %s invalid!' % self.ligand_prmtop)
   ...
   def CheckConsistency(self):
      ...
      for i in range(self.complex_prmtop.ptr('natom')):
         resnum = self.complex_prmtop.residue_container[i] - 1
         atom_in_res = (i + 1 - 
