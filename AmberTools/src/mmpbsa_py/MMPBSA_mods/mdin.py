...
from MMPBSA_mods.cntrl import cntrl
from MMPBSA_mods.ewald import ewald
from MMPBSA_mods.pb import pb
from MMPBSA_mods.qmmm import qmmm
...
class mdin:
   ...
   def __init__(self, program = 'sander', verbosity = 1):
      ...
      if self.program == "sander":
         ...
         raise CreateInputError('Error: program (%s) unrecognized!' % self.program)
   ...
   def write(self, filename = 'mdin'):
      ...
      if not self.check(): # check the validity of the parameters
         raise CreateInputError('mdin file failed sanity check')
      ...
      for i in range(len(lines)):
         if not inblock and not lines[i].strip().startswith('&') and \
            ...
            if not block in self.valid_namelists:
               raise CreateInputError('Invalid namelist (%s) in input file (%s) for %s'
                             % (lines[i].strip(), filename, self.program))
         elif inblock and (lines[i].strip() == '/' or 
                           lines[i].strip() == '&end'):
            inblock = False
         elif inblock and lines[i].strip().startswith('&'):
            raise CreateInputError(('Invalid input file (%s). Terminate each ' +
   ...
   def change(self, namelist, variable, value):
      ...
      if namelist == "cntrl":
         if variable in self.cntrl_nml.keys():
            ...
            raise CreateInputError('Unknown variable (%s) in &cntrl!' % variable)
      elif namelist == 'ewald': 
         if variable in self.ewald_nml.keys():
            self.ewald_nml[variable] = value
         else:
            raise CreateInputError('Unknown variable (%s) in &ewald!' % variable)
      elif namelist == 'pb' or namelist == 'apbs':
         if variable in self.pb_nml.keys():
            self.pb_nml[variable] = value
         else:
            raise CreateInputError('Unknown variable (%s) in &%s!' % 
                            (variable, namelist))
      elif namelist == 'qmmm':
         if variable in self.qmmm_nml.keys():
            self.qmmm_nml[variable] = value
         else:
            raise CreateInputError('Unknown variable (%s) in &qmmm' % variable)
      else:
         raise CreateInputError('Unknown namelist (%s)!' % namelist)
