...
subroutine set_membrane_eps(epsx,epsy,epsz,epsin,epsout,epsmemb,&
    ...
    do zi = max(0,mizmin-1),min(1+mizmax,zm) !only need to loop over z values near membrane
    ...
    do xi = 0,xm; do yi = 0,ym
        ...
        if (p0*px == 0) then
           if (abs(p0)+abs(px) == 0) then !membrane regular point
        ...
        if ( xi > 0 .and. zi>0 ) then
            if (p0*py == 0) then
               if (abs(p0)+abs(py) == 0) then !membrane regular point
        ...
        if ( xi > 0 .and. yi>0) then
            if (p0*pz == 0) then
                if (abs(p0)+abs(pz) == 0) then !membrane regular point
