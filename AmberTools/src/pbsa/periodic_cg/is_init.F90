...
#  include "pb_md.h"
   ...
   integer bcoption, npbverb, i, j, l, phiout, saltout, scalec, alloc_err
   _REAL_ space, coef(1:10,1:4)
    
   ! begin code
 
   namelist /pb/epsin, epsout, istrng, stern, pbtemp, radiopt, sprob, maxsph, &
      maxarc, arcres, smoothopt, &
      fillratio, fillx, filly, fillz, space, nbuffer, offx, offy, offz, &
      accept, maxitn, &
      dbfopt, scalec, npbgrid, nsnbr, nsnba, &
      cutres, cutfd, cutnb, npopt, cavity_surften, cavity_offset, &
      phiout, phiform, saltout, npbverb, &
      pbkappa, radinc, expthresh, &
      ndofd, ndosas, ndosar, nfocus, fscale, solvopt, fmiccg, &
      cutsa, bcoption, &
      sepbuf, mpopt, lmax

   namelist /pbspline/nn, coef
   ...
   if ( mdin_pb ) then
      rewind 5
      read(5, nml=pb) 
      if (igb == 12) read(5, nml=pbspline)
   end if

   allocate ( dash(1:nn), stat = alloc_err )
   if ( alloc_err /= 0 ) then
      write(6, *) 'PB Bomb in pb_read(): Allocation aborted', alloc_err
      call mexit(6, 1)
   endif
   do i = 1, nn
      dash(i) = dble(i-1)/dble(nn-1)
   enddo
   allocate ( spcoef(1:nn-1, 1:4), stat = alloc_err )
   if ( alloc_err /= 0 ) then
      write(6, *) 'PB Bomb in pb_read(): Allocation aborted', alloc_err
      call mexit(6, 1)
   endif
   ...
   enddo
   enddo
   !write(*,*) 'density spline function:', nn
   !do i = 1, nn-1
   !  write(*,'(5f12.5)') dash(i),spcoef(i,1:4)
   !enddo
   !write(*,'(f12.5)') dash(i)
    
   ! assign dielectric model
   ...
   case ( 12 )
      dielopt = 2
   case default
      write(6,"(a)") "PB Bomb in pb_read(): invalid igb option"
      call mexit (1)
   end select
   ...
   if ( dielopt == 0 .and. (ndosar == 0 .or. ndosas == 0) ) then
      write(6,"(a)") "PB Bomb in pb_read(): invalid igb and sar/sas combination", igb, ndosar, ndosas
      call mexit (1)
   end if
   if ( dielopt > 0 .and. (ndosar > 0 .or. ndosas == 0) ) then
      write(6,"(a)") "PB Bomb in pb_read(): invalid igb and sar/sas combination", igb, ndosar, ndosas
   ...
   if ( fillx == ZERO ) fillx = fillratio 
   if ( filly == ZERO ) filly = fillratio 
   if ( fillz == ZERO ) fillz = fillratio 
...
subroutine pb_print ( ifcap, natom )
...
#  include "pb_md.h"
   ...
   if ( bcopt == 1 ) then
      ...
      write(6, *) '  3D periodic boundary condition'
...
#include "box.h"
   ...
   deallocate( radipd, stat = alloc_err(15) )
   ...
   deallocate(  radipd, stat = alloc_err(12) )
...
#  include "pb_md.h"
   ...
   integer ires, iatm, jatm, nsatm, maxmax, ic, i, j, jp, idum
   integer alloc_err(64)
   _REAL_ rinchk
   ...
   allocate( radipd(  natom  ), stat = alloc_err(15) )
   ...
   allocate( radipd(    1: 1   ), stat = alloc_err(15) )
   ...
   if ( ifcap == 0 ) then
      ...
      if ( pbverbose ) then
         ...
         do iatm = 1, natom
            write(6, '(i5,3a6,5f10.6)') iatm,resid(iatm),igraph(iatm),isymbl(iatm),&
      ...
      ! initialization for sas/sar surface

      call sa_sphere( maxsph, minsph, scrd )
