...
integer, public, dimension(:), allocatable  :: jgroup, index
...
subroutine allocate_int_decomp( idecomp, natom, nres )
...
subroutine deallocate_int_decomp( idecomp )
...
subroutine decphi(nat1,nat2,nat3,nat4,fval)
   ...
   logical isprot1, isprot2, isprot3, isprot4
   integer nssel, nsind, nsdir
   integer nbsel, nbind, nbdir
...
#include "memory.h"
   ...
   do i=0,nres-1  ! here: not till nres -> keeps "rest" apart
...
#  include "memory.h"
   ...
   350 format(3x,1x,6hresid ,'|',9hinternal ,'|',9hvdw      ,'|', &
         9heel      ,'|',9hpol      ,'|',9hsas      )
   355 format(6(10h==========))
...
#  include "memory.h"
   ...
   350 format(3x,1x,7hresid1 ,'->',7hresid2 , &
         '|',9hinternal ,'|',9hvdw      ,'|', &
         9heel      ,'|',9hpol      ,'|',9hsas      )
   355 format(7(10h==========))
