...
!           call pb_qefrc( natom, 1, atmlast, idecomp, eelrf, eelself, &
!                          outflag, pbfrc, phi )
!           pbfrc = pbfrc*frcfac
!           open (unit = 103, file = 'force.dat')
!           write(103,*) ' :::: Atomic qE forces ::::'
!           do iatm = 1, natom
!              write(103,'(3e20.6)') pbfrc(1:3,iatm)
!           end do
...
subroutine pb_dbfrc_crg( verbose,eneout,natom,eelrf,f,epsx,epsy,epsz,zv,phi,chgrd,cphi )
   ...
   do ip = 1, nbnd
      ...
      if ( iatm == 0 ) then
         write(6,*) 'PB Bomb in pb_dbfrc_crg(): can not find projection atom/probe'
         call mexit(6, 1)
      end if
...
subroutine pb_dbfrc_fld2( verbose,eneout,natom,eelrf,f,epsx,epsy,epsz,zv,phi,chgrd,cphi )
   ...
   do ip = 1, nbndx
      ...
      ! dr = abs(rn(1))
      ! r1 = ONE/sqrt(rn(1)**2+rn(2)**2+rn(3)**2)
      ! ds = dr*r1*h2
      ! srfarea1 = srfarea1 + ds
...
#  include "pb_constants.h"
   ...
   do ip = 1, nbnd
      ...
      if ( iatm == 0 ) then
         write(6,*) 'PB Bomb in pb_dbfrc_fld3(): can not find projection atom/probe'
         call mexit(6, 1)
      end if
      if      ( iatm > 0 ) then
         ! the contact boundary grid points are projected to the atom spheres
...
!     write(400,'(3I7,f15.6)')i, j, k, crg
      ...
      if ( iatm == 0 ) then
         write(6,*) 'PB Bomb in pb_crgview(): can not find projection atom/probe'
         call mexit(6, 1)
      end if
