			     ...
			     int cgid)
{
  ...
  int nh2, nh2id, nh2reg, nh2id3, ncpu, hsctr;
  int expsctr[8];
  int* cellidx;
  double* xpos;
  double* xvel;
  ...
  for (h = 0; h < 8; h++) {
	...
	/*** CHECK ***/
	if (C->data[nh1].id != nh1id) {
	  printf("Oops, wanted atom %5d but got %5d\n", nh1id,
		 C->data[nh1].id);
	}
	if (C->data[nh2].id != nh2id) {
	  printf("Oops, wanted atom %5d but got %5d\n", nh2id,
		 C->data[nh2].id);
	}
	/*** END CHECK ***/
...
static void BroadcastVelocities(cellgrid *CG, int cellid, int imove, int nmsg)
{
  ...
  aexp = 1;
  texport = C->export;
  C->export = (C->CGRank == D->CGRank) ? D->import :
    &CG->export[nmsg][CG->nexp[nmsg]];
  for (i = 0; i < C->nexp; i++) {
    oreg = C->Vexport[i].dreg;
    if (oreg == iC1 || oreg == iC2 || oreg == iC3 || oreg == iC4) {
      C->export[aexp].id = C->Vexport[i].id;
      C->export[aexp].loc[0] = C->Vexport[i].vel[0];
      C->export[aexp].loc[1] = C->Vexport[i].vel[1];
      C->export[aexp].loc[2] = C->Vexport[i].vel[2];
      C->export[aexp].dreg = (oreg == iC1) ? iD1 : (oreg == iC2) ? iD2 :
	(oreg == iC3) ? iD3 : iD4;
      aexp++;
    }
  }
