...
/***=======================================================================***/
static void InitHistory(coord *crd)
{
  int i;
		...
		Energy *sysUV, execon *etimers, trajcon *tj)
{
  ...
  for (i = 0; i <= CG->MyCellCount; i++) {
    ...
    DirectTriple2(C, crd, CG, Etab, dcinp, tp, sysUV);
    etimers->nbInt += mdgxStopTimer(etimers);
...
{
  int i, j, k, gcon, g3con;
  double pmovex, pmovey, pmovez;
  double *ctmp, *ltmp, *vtmp, *ftmp, *mtmp, *masstmp, *pftmp;
  cell *C;
  ...
  for (i = 0; i < CG->MyCellCount; i++) {
    ...
    if (tj->ntt == 3) {
      ...
      for (j = 0; j < C->nr[0]; j++) {
	gcon = C->data[j].id;
	const double rsd = sdfac * sqrt(masstmp[gcon]);
	const double hmdt = hdt*mtmp[gcon];
	ctmp = C->data[j].loc;
	ftmp = C->data[j].frc;
	g3con = 3*gcon;
	vtmp = &crd->vel[g3con];
	...
	ltmp[2] += pmovez;
      ...
      for (j = 0; j < C->nr[0]; j++) {
	const double hmdt = hdt*mtmp[C->data[j].id];
	ctmp = C->data[j].loc;
	ftmp = C->data[j].frc;
	g3con = 3*C->data[j].id;
	vtmp = &crd->vel[g3con];
...
#endif
  for (i = 0; i < CG->ng[0]*CG->ng[1]*CG->ng[2]; i++) {
    DirectTriple2(&CG->data[i], crd, CG, Etab, dcinp, &tpX, &sysUVt);
    CellBondedIntr(&CG->data[i], CG, crd, &tpX, Etab, EHtab, &sysUVt);
  }
#ifdef MPI
  MergeCellForces(CG, crd, tp, tj);
