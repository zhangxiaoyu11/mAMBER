...
void ReadEPRuleFile(prmtop *tp)
{
  ...
  while (AdvanceToSegment(inp, "rule", 0) != 0) {
    ...
    while (collect == 1) {
      ...
      RemoveComments(line);
      collect = DetectNamelistEnd(line, "ReadEPRuleFile");
      if (collect == 0) {
        continue;
      }
...
static void CellPlaceXpt(prmtop *tp, coord *crd, cell *C, cellgrid *CG)
{
  ...
  for (h = 0; h < 8; h++) {
    for (i = 0; i < C->nr[h]; i++) {
      ...
      if (hiFR1->natom == 0 ||
	  (hsctr = IsCentralAtom(C->orig, ccen, atmhi->loc,
			  ...
			  int iD1, int iD2, int iD3, int iD4, double* reimv)
{
  ...
  for (i = C->nr[iC1]; i < C->nr[iC1] + C->nr[iC1+8]; i++) {
    LoadEP4Export(&C->map[iC1][i], &C->export[nexp], iD1, reimv);
    nexp++;
  }
  for (i = C->nr[iC2]; i < C->nr[iC2] + C->nr[iC2+8]; i++) {
    LoadEP4Export(&C->map[iC2][i], &C->export[nexp], iD2, reimv);
    nexp++;
  }
  for (i = C->nr[iC3]; i < C->nr[iC3] + C->nr[iC3+8]; i++) {
    LoadEP4Export(&C->map[iC3][i], &C->export[nexp], iD3, reimv);
    nexp++;
  }
  for (i = C->nr[iC4]; i < C->nr[iC4] + C->nr[iC4+8]; i++) {
    LoadEP4Export(&C->map[iC4][i], &C->export[nexp], iD4, reimv);
    nexp++;
  }
  C->export[0].id = nexp;
  C->nexp = nexp;
...
#endif
{
  ...
  for (imove = 0; imove < 3; imove++) {
	...
	for (j = 0; j < cshr2->send[i].ncell; j++) {
	  ...
	  for (k = 0; k < O->export[0].id; k++) {
	    N->import[k] = O->export[k];
	  }
	}
#ifdef MPI
      }
...
{
  int h, i, j, k, epid, bid, cid, did, hsctr;
