...
/***=======================================================================***/
void PrintAtomState(cellgrid *CG, coord *crd, prmtop *tp, int atmid)
{
  int i, atmcid;
  double *ftmp;
  cell *C;
		 ...
		 trajcon *tj, MPI_Datatype msgtype)
{
  ...
  for (j = 0; j < cshr->send[isend].ncell; j++) {
	...
	sprintf(ctmp, "%6d ", CG->Xexport[isend][xpcon+k].id);
      ...
      for (k = 1; k < CG->export[isend][xpcon].id; k++) {
	ctmp = &outline[strlen(outline)];
	sprintf(ctmp, "%6d ", CG->export[isend][xpcon+k].id);
      }
      ctmp = &outline[strlen(outline)];
      sprintf(ctmp, "\n");
    }
  ...
  if (msgtype == tj->MPI_ATOMX) {
    ...
    for (j = 0; j < msgsize; j++) {
      ctmp = &outline[strlen(outline)];
      sprintf(ctmp, "%6d ", CG->export[isend][j].id);
    }
  }
  ctmp = &outline[strlen(outline)];
  sprintf(ctmp, "];\n");
