...
struct BondMap {
  ...
  double* kernel;   // The kernel x stiffness constant = contribution
  double* contrib;  // Contributions from each bond term (stored WITHOUT bond
                    //   stiffness factors, for easy entry into the fitting
                    //   matrix)
};
...
struct AngleMap {
  ...
  double* kernel;   // The kernel x stiffness constant = contribution
  double* contrib;  // Contributions from each angle term (stored WITHOUT angle
                    //   stiffness factors, for easy entry into the fitting
                    //   matrix)
};
...
struct TorsionMap {
  ...
  double* val;      // Values of the underlying angle in the fitting set
  double* kernel;   // The kernel x amplitude = contribution
  double* contrib;  // Contributions from each dihedral term (stored WITHOUT
                    //   torsion stiffness factors, for easy entry into the
                    //   fitting matrix)
};
...
struct MMSystem {
  ...
  double EEkernel; // The kernel for adjustable electrostatic interactions 
  double LJkernel; // The kernel for adjustable Lennard-Jones interactions 
  ...
  char crdsrc[MAXNAME];    // The source file for the coordinates (inpcrd 
                           // format)
};
