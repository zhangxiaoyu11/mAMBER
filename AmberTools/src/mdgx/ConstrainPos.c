...
#endif
{
  ...
  for (h = 0; h < 8; h++) {
	...
	RattleGroupCnst(CG, crd, tj, tp, C, C->gbin, tSHL[atmhi->id].blist,
			cellidx, xref, xpos, xposc, hsctr);
#elif CNSTTYPE == 1
	RattleGroupRscl(CG, crd, tp, C, C->gbin, tSHL[atmhi->id].blist,
			cellidx, xref, xpos, xposc, chi, invchi, hsctr);
#endif
      }
    }
  }
...
#endif
{
  ...
  for (i = 1; i < D->import[0].id; i++) {
    ...
    if (dRimp->dreg == 0) {
      ...
      /*** CHECK ***/
      if (D->data[D->GPSptr[atmid]].id != atmid) {
	printf("Oops, got %5d when I wanted %5d.\n",
	       D->data[D->GPSptr[atmid]].id, atmid);
      }
      /*** END CHECK ***/
