...
{
  int i, j, k, ngps, ix, iy, iz, nbx, nby, nbz;
  int cdim[3];
  double cdepth[3];
  cellgrid CG;
...
{
  int i, j, i4, i4p1, i4p2, i8, maxatom, idR;
  int *cnsr, *itmp, *itm2p, *cordr;
  double invcut, invEEcut, dR;
  double **fcn;
...
static void ManageIntr(cell *C, int sA, int sB, int oA, int oB, FrcTab *EFrc,
		       dircon *dcinp, prmtop *tp, Energy *sysUV, int isortho)
{
  int i, cstr;
...
/***   CG:      the cell grid (for computing cell midpoints)               ***/
...
/***=======================================================================***/
void DirectTriple2(cell *C, coord *crd, cellgrid *CG, FrcTab *EFrc,
		   dircon *dcinp, prmtop *tp, Energy *sysUV)
{
  ...
  if (dcinp->Vcut > dcinp->Ecut) {
    ...
    ManageIntr(C, 0, 1,  0,   4, EFrc, dcinp, tp, sysUV, 0);
    ManageIntr(C, 0, 2,  8,  12, EFrc, dcinp, tp, sysUV, 0);
    ManageIntr(C, 0, 4, 16,  20, EFrc, dcinp, tp, sysUV, 0);

    /*** Next, do interactions across edges (6) ***/
    ManageIntr(C, 0, 3, 24,  28, EFrc, dcinp, tp, sysUV, 0);
    ManageIntr(C, 0, 5, 32,  36, EFrc, dcinp, tp, sysUV, 0);
    ManageIntr(C, 0, 6, 40,  44, EFrc, dcinp, tp, sysUV, 0);
    ManageIntr(C, 1, 2, 48,  52, EFrc, dcinp, tp, sysUV, 0);
    ManageIntr(C, 1, 4, 56,  60, EFrc, dcinp, tp, sysUV, 0);
    ManageIntr(C, 2, 4, 64,  68, EFrc, dcinp, tp, sysUV, 0);

    /*** Finally, do interactions across the center point (4) ***/
    ManageIntr(C, 0, 7, 72,  76, EFrc, dcinp, tp, sysUV, 0);
    ManageIntr(C, 1, 6, 80,  84, EFrc, dcinp, tp, sysUV, 0);
    ManageIntr(C, 2, 5, 88,  92, EFrc, dcinp, tp, sysUV, 0);
    ManageIntr(C, 4, 3, 96, 100, EFrc, dcinp, tp, sysUV, 0);
  }
  else {
    ...
    /*** Next, do interactions across faces (3) ***/
    ManageIntr(C, 0, 1,  0,   4, EFrc, dcinp, tp, sysUV, 1);
    ManageIntr(C, 0, 2,  8,  12, EFrc, dcinp, tp, sysUV, 1);
    ManageIntr(C, 0, 4, 16,  20, EFrc, dcinp, tp, sysUV, 1);

    /*** Next, do interactions across edges (6) ***/
    ManageIntr(C, 0, 3, 24,  28, EFrc, dcinp, tp, sysUV, 1);
    ManageIntr(C, 0, 5, 32,  36, EFrc, dcinp, tp, sysUV, 1);
    ManageIntr(C, 0, 6, 40,  44, EFrc, dcinp, tp, sysUV, 1);
    ManageIntr(C, 1, 2, 48,  52, EFrc, dcinp, tp, sysUV, 1);
    ManageIntr(C, 1, 4, 56,  60, EFrc, dcinp, tp, sysUV, 1);
    ManageIntr(C, 2, 4, 64,  68, EFrc, dcinp, tp, sysUV, 1);

    /*** Finally, do interactions across the center point (4) ***/
    ManageIntr(C, 0, 7, 72,  76, EFrc, dcinp, tp, sysUV, 1);
    ManageIntr(C, 1, 6, 80,  84, EFrc, dcinp, tp, sysUV, 1);
    ManageIntr(C, 2, 5, 88,  92, EFrc, dcinp, tp, sysUV, 1);
    ManageIntr(C, 4, 3, 96, 100, EFrc, dcinp, tp, sysUV, 1);
  }
}
...
{
  int i, j;
  int nrEE[8], nrLJ[8], nboth[8];
  double nPairList, nR2;
  ...
  /*** Estimate the pair list building cost ***/
  nPairList = C->nr[0]*7 + (C->nr[1] + C->nr[2] + C->nr[4])*4 +
    (C->nr[3] + C->nr[5] + C->nr[6])*2 + C->nr[7];
