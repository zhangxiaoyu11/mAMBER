...
#define WT_BL           0.5	//penalty weight for bond length
#define WT_BLF          0.5	//penalty weight for bond stretching force constant
#define WT_BA           0.5	//penalty weight for bond angle
#define WT_BAF          0.5	//penalty weight for bond angle bending
#define WT_X 	        10	//penalty for X in 1,2 and 4 th positions of improper parameters
#define WT_3X           30	//penalty for X in the third place of improper parameters
#define WT_ANGLE_CENTER 10 	//additional penalty weight of replacing angle central atom: penalty of the center atom = WT_BA/WT_BAF * WT_ANGLE_CENTER
#define WT_TOR_CENTER   10	//additional penalty weight of replacing torsional central atoms: penalty of the center atoms = WT_TOR * WT_TOR_CENTER
#define WT_IMPROPER     10      //additional penalty weight for  the third atom in an improper angle, weights for atoms in the 1, 2 and 4th places are 1.0 
#define WT_GROUP        100	//additional penalty of different atom type groups: total weight = total weight + WT_GROUP
#define THRESHOLD_BA    20	//threshold triggling emprical calculations
#define INITSCORE       999999

# define DEFAULT_BL   20		//default penalty for bond length
# define DEFAULT_BLF  30 		//default penalty for bond streching force constant
# define DEFAULT_BA2  13		//default penalty for center atom of bond angle
# define DEFAULT_BAF2 17 		//default penalty for center atom of bond angle 
# define DEFAULT_BA   4		//default penalty for bond angle
# define DEFAULT_BAF  15		//default penalty for bond angle
# define DEFAULT_TOR2 95 		//default penalty for torsional angle
# define DEFAULT_TOR  54		//default penalty for torsional angle
...
void read_parmchk_parm(char *filename)
{
	...
	int  ncorr;
	int  nequa;
	double bl,blf,ba,baf, cba, cbaf, tor, ctor, itor;
	double mass;
	if ((fp = fopen(filename, "r")) == NULL) {
	...
	for (;;) {
		...
		if (strncmp("PARM", &line[0], 4) == 0) {
			sscanf(&line[4], "%s%d%d%lf", atomtype, &improper, &group, &mass) ;
...
/*      Equal atom types must alos be corresponding atom types */
		...
		if (strncmp("CORR", &line[0], 4) == 0) {
			...
			sscanf(&line[4], "%s%lf%lf%lf%lf%lf%lf%lf%lf%lf",  
                                         corrname, &bl, &blf, &cba, &cbaf, &ba, &baf, &ctor, &tor, &itor); 
			strcpy(parm[parmnum-1].corr[ncorr].atomtype, corrname);
			if(bl  < 0) bl  = DEFAULT_BL;
			if(blf < 0) blf = DEFAULT_BLF;
			if(ba  < 0) ba  = DEFAULT_BA;
			if(baf < 0) baf = DEFAULT_BAF;
			if(cba < 0) cba = DEFAULT_BA2;
			if(cbaf< 0) cbaf= DEFAULT_BAF2;
			if(ctor< 0) ctor= DEFAULT_TOR2;
			if(tor < 0) tor = DEFAULT_TOR;
			...
			parm[parmnum-1].corr[ncorr].improper=itor;
			parm[parmnum-1].corr[ncorr].pid = -1;
...
/*	now finding the pid of equal/corresponding atom types */
	...
	if(debug) {
		for(i=0;i<parmnum;i++) {
			printf("PARM %5d %5s %5d %5d %9.4lf\n", i+1, parm[i].atomtype, parm[i].improper, parm[i].group, parm[i].mass);
			for(j=0;j<parm[i].nequa;j++)
				printf("EQUA %5d %5s %5d \n", 
				j+1, parm[i].equa[j].atomtype, parm[i].equa[j].pid + 1); 
			for(j=0;j<parm[i].ncorr;j++)
				printf("CORR %5d %5s %5d %5.1lf %5.1lf %5.1lf %5.1lf %5.1lf %5.1lf %5.1lf %5.1lf %5.1lf %5d\n", 
				...
				parm[i].corr[j].improper, parm[i].corr[j].type); 
			printf("\n");
		}
	}
}
...
void chk_vdw(void)
{
	...
	for (i = 0; i < atomnum; i++) {
		...
		if (suc == 0 && parm[pid].nequa > 1) 
			for(j=1; j< parm[pid].nequa; j++) {
				pid2 = parm[pid].equa[j].pid;	
				suc = vdw(atom[i].ambername, parm[pid2].atomtype);
				if(suc == 1) break;
			}
		if (suc == 0 && parm[pid].ncorr > 1) 
			for(j=1; j< parm[pid].ncorr; j++) {
					...
					bestblid = -1;
						...
						if(suc == 1) break;
							...
							if(suc == 1) break;
                                                        ...
                                                        suc2 = bond(name1, name2, tmpc1, tmpc2, 1);
							if(suc2 == 1) {
								bestscore = 0;
								suc = 1;
							}
...
//	for corresponding atom types
				if(suc == 0) {
					...
					for(m=0;m<parm[pid1].ncorr; m++) {
						...
						strcpy(tmpc1, parm[pid3].atomtype);	
						score1= parm[pid1].corr[m].bl * WT_BL +
						        parm[pid1].corr[m].blf* WT_BLF;
						for(n=0;n<parm[pid2].ncorr; n++) {
							...
							strcpy(tmpc2, parm[pid4].atomtype);	
							score2= parm[pid2].corr[n].bl * WT_BL +
						        	parm[pid2].corr[n].blf* WT_BLF;
							score = score1 + score2;
							if(parm[pid3].group != parm[pid4].group) score += WT_GROUP;
							if(score < bestscore) {
								suc2= bond(name1, name2, tmpc1, tmpc2, 1);
								if(suc2== 1) {
									bestscore = score;
									...
									if(suc == 1) break;
										...
										if(suc == 1) break;
											...
											if(suc == 1) break;
                                                                                        ...
                                                                                        suc2 = angle(name1, name2, name3, tmpc5, tmpc4, tmpc6, 1);
											if(suc2 == 1) {
												bestscore = 0;
												suc = 1;
											}
...
/*	for corresponding atom types */
								...
								bestbaid = -1;
                                        			...
                                        			for(m=0;m<parm[pid1].ncorr; m++) {
                                                			...
                                                			strcpy(tmpc4, parm[pid4].atomtype);
                                                			score1= parm[pid1].corr[m].bl * WT_BA +
                                                        			parm[pid1].corr[m].blf* WT_BAF;
                                                			for(n=0;n<parm[pid2].ncorr; n++) {
                                                        			pid5 = parm[pid2].corr[n].pid;
                                                        			strcpy(tmpc5, parm[pid5].atomtype);
                                                        			score2= parm[pid2].corr[n].bl * WT_BA +
                                                                			parm[pid2].corr[n].blf* WT_BAF;
										score2 *= WT_ANGLE_CENTER;
											...
											if(parm[pid1].corr[m].type <= 1 && 
												...
												continue;	
                                                        				pid6 = parm[pid3].corr[o].pid;
                                                        				strcpy(tmpc6, parm[pid6].atomtype);
                                                        				score3= parm[pid3].corr[o].bl * WT_BA +
                                                                				parm[pid3].corr[o].blf* WT_BAF;
											score = score1 + score2 + score3 + WT_GROUP;
											if(parm[pid4].group == parm[pid5].group && 
											   parm[pid4].group == parm[pid6].group)
                                                                                        	score -= WT_GROUP;
                                                        				if(score < bestscore && score <=THRESHOLD_BA) {
                                                                				suc2= angle(name1, name2, name3, tmpc4, tmpc5, tmpc6, 1);
                                                                				if(suc2== 1) {
													bestscore = score;
													suc = 1;
												}
...
// Step 1 check if the special torsional parameter exists or not
									suc = torsion(name1, name2, name3, name4,
											tmpc1, tmpc2, tmpc3, tmpc4, 0);
									if(suc == 1) continue;
// Step 2 check special torsional parameters using equal atom types
                                                                        if (suc == 0) {
                                                                        	besttorid = -1;
                                                                                pid1 = parmid[j]; //central atoms in the first tow for loops 
                                                                                pid2 = parmid[k];
                                                                                pid3 = parmid[i];
                                                                                pid4 = parmid[l];
                                                                                for(m=0; m<parm[pid1].nequa; m++) {
												...
												if(suc == 1) break;
													...
													if(suc == 1) break;
														...
														if(suc == 1) break;
                                                                                                                ...
                                                                                                                suc2= torsion(name1, name2, name3, name4, tmpc7, tmpc5, tmpc6, tmpc8, 1);
														if(suc2== 1) {
															bestscore = 0;
															suc = 1;
														}
...
// Step 4 check general torsional angle terms using equal atom types
												...
												if(n==0 && p==0) continue;
                                                                                                pid7 = parm[pid3].equa[p].pid;
                                                                                                strcpy(tmpc7, parm[pid7].atomtype);
                                                                                                suc2= torsion(name1, name2, name3, name4, "X", tmpc6, tmpc7, "X", 1);
												if(suc2== 1) {
													bestscore = 0;
													suc = 1;
												}
										...
										for(m=0; m<parm[pid1].ncorr; m++) {
											pid5 = parm[pid1].corr[m].pid;
												...
												pid6 = parm[pid2].corr[n].pid;
                                                                        			strcpy(tmpc6, parm[pid6].atomtype);
                                                                        			score2= parm[pid2].corr[n].tor * WT_TOR_CENTER; 
                                                                        			for(p=0;p<parm[pid3].ncorr; p++) {
													pid7 = parm[pid3].corr[p].pid;
                                                                        				strcpy(tmpc7, parm[pid7].atomtype);
                                                                        				score3= parm[pid3].corr[p].tor * WT_TOR_CENTER; 
														...
														if(parm[pid5].group == parm[pid6].group && 
															...
															score -= WT_GROUP;
														if(score < bestscore) {
															suc2= torsion(name1, name2, name3, name4, tmpc5, tmpc6, tmpc7, tmpc8, 1);
															if(suc2== 1) {
																bestscore = score;
																suc = 1;
															}
...
//	step 3 considering general atom types 
		if (suc == 0) {
			...
			for (j = 0; j < improperparmnum; j++) {
				...
				score3 = 0;
				score4 = 0;
				if(strcmp(improperparm[j].name1, "X") == 0) score1 = WT_X;
				if(strcmp(improperparm[j].name2, "X") == 0) score2 = WT_X;
				if(strcmp(improperparm[j].name3, "X") == 0) score3 = WT_3X;
				if(strcmp(improperparm[j].name4, "X") == 0) score4 = WT_X;
				score = score1 + score2 + score3 + score4;
				if(score < bestscore) {
					bestimproperid = j;
					bestscore = score;
                ...
                if(suc == 0) {          
                        ...
                        for(m=0; m<parm[pid1].nequa; m++) {
							...
							if(m==0 && n==0 && p==0 && q==0) continue;  
								...
								score3 = 0;
								score4 = 0;
                                                                if(strcmp(improperparm[k].name1, "X") == 0) score1= WT_X;
                                                                if(strcmp(improperparm[k].name2, "X") == 0) score2= WT_X;
                                                                if(strcmp(improperparm[k].name3, "X") == 0) score3= WT_3X;
                                                                if(strcmp(improperparm[k].name4, "X") == 0) score4= WT_X;
								score = score1 + score2 + score3 + score4;
                                                                if(score < bestscore) {
                                                                        bestimproperid = k;
                                                                        bestscore = score;
                                ...
                                for(n=0;n<parm[pid2].ncorr; n++) {
                                	...
                                	score2= parm[pid2].corr[n].improper;
                                        for(p=0;p<parm[pid3].ncorr; p++) {
                                                pid7 = parm[pid3].corr[p].pid;
                                                strcpy(tmpc3, parm[pid7].atomtype);
                                		score3= parm[pid3].corr[p].improper * WT_IMPROPER;
                                                for(q=0;q<parm[pid4].ncorr; q++) {
                                                	if(parm[pid1].corr[m].type <= 1 &&
                                                           parm[pid2].corr[n].type <= 1 &&
                                                           parm[pid3].corr[p].type <= 1 &&
			...
			bestimproperid = -1;
                        for(m=0; m<parm[pid1].ncorr; m++) {
							...
							for (k = 0; k < improperparmnum; k++){
								...
								if(strcmp(improperparm[k].name4, tmpc4) != 0 && strcmp(improperparm[k].name4, "X") != 0) 
									continue;	
								if(strcmp(improperparm[k].name1, "X") == 0) score += WT_X;
								if(strcmp(improperparm[k].name2, "X") == 0) score += WT_X;
								if(strcmp(improperparm[k].name3, "X") == 0) score += WT_3X;
								if(strcmp(improperparm[k].name4, "X") == 0) score += WT_X;
								if(score < bestscore) {
									bestimproperid = k;
                                                               		bestscore = score;
									suc = 1;
	...
	if (strcmp(COLORTEXT, "YES") == 0 || strcmp(COLORTEXT, "yes") == 0) {
		if (argc == 2
			...
			printf("[31mUsage: parmchk -i [0m input file name\n"
				   ...
				   "[31m               -c [0m atom type corresponding file, default is PARMCHK.DAT\n" 
		...
		if (argc == 2
			...
			printf("               -c   atom type corresponding file \n");
