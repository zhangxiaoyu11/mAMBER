...
from os import path
import re
from updateutils.downloader import needs_internet
from updateutils.exceptions import (AmberUpdaterWarning, UpdaterTypeError,
                                    PatchingError)
   ...
   def apply(self, idx, reverse=False, ignore_prev_applied=True):
      ...
      if idx == 'all':
         for idx in range(len(self.names)):
            ...
            p.safe_apply(global_state, reverse, ignore_prev_applied)
      else:
         p = self._instantiate(idx)
         return p.safe_apply(global_state, reverse, ignore_prev_applied)
